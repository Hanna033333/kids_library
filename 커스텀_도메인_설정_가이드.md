# 커스텀 도메인 설정 가이드
`api.bookbangu.com` → Render 백엔드 연결하기
...
```powershell
nslookup api.bookbangu.com
```

### 3-2. 온라인 도구로 확인
- https://dnschecker.org 접속
- `api.bookbangu.com` 입력
- CNAME 레코드가 표시되면 성공!
...
2. `api.bookbangu.com` 옆에 **초록색 체크** 또는 **Verified** 표시 확인
...
4. **Edit** 클릭
5. 값을 `https://api.bookbangu.com`으로 변경
6. **Save** 클릭
...
### 6-2. API 문서 확인
```
https://api.bookbangu.com/docs
```
FastAPI Swagger 문서가 표시되면 성공!

### 6-3. 프론트엔드에서 테스트
1. https://bookbangu.com 접속
2. 책 검색 또는 목록 조회
3. 개발자 도구(F12) → Network 탭
4. API 요청이 `api.bookbangu.com`으로 가는지 확인

---

## ✅ 완료 체크리스트

- [ ] Render에 `api.bookbangu.com` 추가 완료
- [ ] 후이즈 DNS CNAME 레코드 추가 완료
- [ ] DNS 전파 확인 (nslookup 또는 dnschecker)
- [ ] Render SSL 인증서 발급 확인
- [ ] Vercel 환경변수 업데이트 완료
- [ ] Vercel 재배포 완료
- [ ] `https://api.bookbangu.com` 접속 테스트 성공
