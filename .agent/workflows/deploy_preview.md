...
위의 1~3단계 검증이 100% 통과되면, 변경 사항을 반드시 아래 순서대로 검토 후 커밋합니다.

```bash
# [필수] 변경 내용 사전 검토 — git add . 전에 반드시 실행
git diff
```

> [!IMPORTANT]
> **`git diff` 결과를 반드시 육안으로 확인한다.** 이 대화 세션에서 의도하지 않은 변경(이전 세션의 미커밋 잔여물, 자동 수정 등)이 포함되어 있으면 해당 파일만 `git restore <file>`로 되돌린 뒤 커밋한다. `git add .`는 미검토 변경사항을 통째로 올리는 위험한 명령이므로, diff 확인 없이 절대 실행하지 않는다.
