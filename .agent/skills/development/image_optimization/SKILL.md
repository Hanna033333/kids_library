...
## 6. 표지 이미지 유효성 검증 및 홈/큐레이션 노출 정책 (No-Image 차단)
- **홈/큐레이션 노출 금지 원칙**: 홈 화면(큐레이션, 교과서 수록도서, 방학 추천, 칼데콧 등)의 모든 도서 카드에는 실제 표지 이미지가 존재하는 도서만 노출되어야 한다. 표지 이미지가 없거나 플레이스홀더인 도서는 사용자 경험 및 신뢰도를 저해하므로 노출 대상에서 원천 배제한다.
- **`noimg` 플레이스홀더 패턴 차단**:
  - 알라딘, 교보, 예스24 등 도서 API가 표지 미존재 시 반환하는 기본 플레이스홀더 이미지(`noimg_sum_b.gif`, `noimg_b.gif`, `noimg`, `nothumb`, `placeholder` 등)는 DB 수집 시 저장하지 않고 `NULL`로 처리한다.
  - 프론트엔드/백엔드 쿼리(`visibleBooks`, `getCaldecottBooks` 등)에서는 `.not('image_url', 'is', null).neq('image_url', '').not('image_url', 'ilike', '%noimg%')` 필터를 공통 적용한다.
  - UI 컴포넌트 렌더링 전 `isValidCoverImage(book.image_url)` 유틸리티를 통해 유효하지 않은 도서는 렌더링 목록에서 제외하는 다중 안전장치를 유지한다.
