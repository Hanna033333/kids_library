...
description: 책자리 프로젝트 개발 워크플로우, 배포 원칙 및 환경 설정 가이드
---

# 책자리 개발 워크플로우

책자리 프로젝트를 개발할 때는 항상 다음 문서를 참고하세요:

## 1. 프로젝트 계획서 확인
먼저 `docs/project_plan.md`를 확인하여 프로젝트의 전체 구조와 원칙을 파악합니다.
...
## 2. 개발 원칙
- **데이터 정확도보다 접근성 우선**
- **크롤링 정확도**: 도서 매칭 시 `제목` + `저자` + `출판사` 일치를 1순위로, `제목` + `저자`를 2순위로 하여 정확도를 높여야 합니다. (단순 첫 번째 결과 선택 금지)우선**
- **자동화보다 검증 가능한 흐름 유지**
...
98: 3. **프로덕션 배포** → 검증 완료 후 `main` 브랜치 push
99: 
100: ### 배포 후 필수 검증 (QA)
101: 배포가 완료되면 반드시 다음 워크플로우를 실행하여 상태를 점검하세요:
102: 
103: - **Dev 배포 후**: `/verify_dev` (개발 환경 건강 체크)
104: - **Prod 배포 후**: `/verify_prod` (상용 환경 건강 체크)
