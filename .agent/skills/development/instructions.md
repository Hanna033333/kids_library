...
5. **웹 크롤링:** 도서관 웹사이트에서 청구기호 등 데이터를 자동 수집해. (상세: `web_crawling.md`)
6. 업무 수행 시 같은 폴더에 있는 development.md를 반드시 준수해.
<<<<<<< HEAD
야 할 핵심 기능을 우선순위에 따라 나열해.
=======
7. 책 이미지 데이터 관련 작업 시에는 `image_guide.md`의 화질 가이드를 준수해.
>>>>>>> dev

## 핵심
1. Next.js와 FastAPI 구조를 숙지해. 특히 Data4Library API와 판교도서관 스크래핑 시 타임아웃과 캐싱(5분) 처리가 project_plan대로 구현되었는지 검토해.
2. 업무 수행 시 같은 폴더에 있는 development.md의 기술 표준을 반드시 준수해.
