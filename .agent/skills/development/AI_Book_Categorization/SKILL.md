...
### 2. 도서 샵(#) 테마 태그 병합 및 보강 정책 [CRITICAL]
- **태그 뱃지 노출 극대화**: 큐레이션을 새로 등록하거나 보강할 때, 책 표지 위와 상세 페이지에 샵(#) 테마 뱃지(예: `#생명존중`, `#판타지`, `#장애` 등)가 빈 칸 없이 잘 보이도록 보강 딕셔너리를 활용해 데이터를 일치시킵니다.
- **데이터 덮어쓰기 금지**: 
  - 신규 큐레이션 등록 시 기존 도서가 이미 DB에 등록되어 있을 수 있습니다.
  - 이 경우 기존의 `curation_tag` (예: `#자연관찰,#우정`)를 완전히 덮어씌워 날리지 않도록, 기존의 태그 문자열에 새 큐레이션 및 테마 태그를 **쉼표로 이어 붙이는 방식(병합 업데이트)**을 엄격히 고수하여 데이터 유실을 차단합니다.
  - (예: 기존 `#자연관찰` + 여름방학2026,#생명존중 ➡️ `여름방학2026,#생명존중,#자연관찰`)

### 3. 정렬 일관성 유지 정책 (Sorting Consistency) [CRITICAL]
...
### 4. 전문 사서의 추천 포인트 (curation_note) 생성 가이드
