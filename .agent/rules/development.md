...
27. **네이버 블로그 원고 자동 생성기 및 카드뉴스 연동 규격**:
    - `backend/generate_blog_post.py`는 주간 큐레이션 도서 데이터를 활용하여 네이버 스마트에디터 ONE에 바로 복사 가능한 3단(제목/본문/태그) 순수 텍스트 원고를 생성한다.
    - 요일별 큐레이션 자동 매핑(KST 기준 월:1번째, 수:2번째, 금:3번째, 그 외:월요일 큐레이션 폴백)을 지원하며, 스레드 연동 도서가 5권 미만일 경우 DB에서 동일 태그 도서를 보충하여 총 5권(1,800자 이상)을 확보한다.
    - **카드뉴스 자동 렌더링**: 각 도서별로 1080x1080 고화질 카드뉴스 이미지를 생성(`output/blog_posts/images/{tag}/card_N.png`)하고 원고에 1:1로 파일 경로를 매핑한다.
    - **HTML 엔티티 및 마크다운 안전 정제**: 도서 데이터에 포함된 `&lt;`, `&gt;` 등의 HTML 엔티티는 책 제목용 유니코드 꺾쇠(`〈`, `〉`)로 올바르게 디코딩하고, `clean_for_naver_editor`를 통해 마크다운 특수문자(`###`, `**`, `---` 등)가 원고에 노출되지 않도록 완벽히 소독한다.
    - **카드 설명글 어절 단위 안전 트리밍**: `text_trimmer.py`는 단어를 글자 수 단위로 무자비하게 자르지 않고, 어절(공백) 단위 및 완성된 문장 종결(55~65자)로 다듬어 글자가 끊기지 않도록 보장한다.
    - **스레드(Threads) 피드 연동 원칙**: 블로그 원고에 수록할 책 목록은 임의로 DB 쿼리를 수행하지 않고, `threads_feeds` 테이블에서 해당 큐레이션 태그의 가장 최근 피드에 등록된 `book_ids`를 우선적으로 조회 및 매핑하여 동일한 도서로 구성되도록 구현한다. 스레드 기록이 존재하지 않는 경우에만 기존의 큐레이션/도서관 조건 기반 쿼리로 폴백한다.
28. **동적 큐레이션 노이즈 정제**:
    - `remove_curation_noise.py` 등을 실행해 정제하되, 정제 이후에도 각 테마의 도서 수량이 7권 이상으로 유지되는지 안전장치를 확인해야 한다.
...
30. **스레드 Gemini 프롬프트 및 파이프라인 개발 표준**:
    ...
    - 스레드 발행 시 첫 댓글 링크 자동 등록 기능은 마케팅 전략 변경에 따라 완전히 중단(주석화)한다. 대신 캡션 마지막 문구를 '스하리' 유도용으로 고정하여 답글 작성을 장려하며, 카드뉴스 노출 도서 권수는 기존 5권에서 3권으로 축소하여 3장만 생성해 올린다.
...
36. **도서관 추가 및 확장 규격**:
    ...
    - 청구기호 수집기 구현 시 어린이 도서 특성을 감안해 제목 유사도 매칭을 배제하고 오직 ISBN 13자리 1:1 매칭만을 신뢰하도록 작성하며, Data4Library API 연동 시에는 공통 분류번호만 반환하는 `srchDtlList` 대신 별치기호/저자기호/복본기호가 모두 보장되는 **`itemSrch` API**를 필수 사용하여 청구기호를 수집 및 조합한다.
    - DB 제약조건 검증 로직 작성 시 외래키 위반 예방을 위해 실제 DB 내 도서 ID를 쿼리해 활용하는 방식을 고수한다.
    - 대량 배치 작업의 안정성 확보를 위해 수집 중단된 도서관 및 book_id 상태를 실시간 json으로 기록하고 `--resume` 옵션으로 이어받을 수 있는 멱등성 설계를 필히 반영한다.
...
6. **개인정보 보호 (어린이 및 보호자 대상 서비스)**
   - **최소 수집 원칙 및 소셜 로그인 현실화:** 카카오 간편 가입 및 소셜 로그인을 통해 닉네임과 비식별화된 이메일 등 서비스 연동에 필요한 최소한의 간접 정보만 수집하며, 어린이에 관한 식별성 있는 정보(실명, 나이, 주민번호 등)는 저장하지 않습니다.
   - **COPPA 준수:** 13세 미만 아동에 대한 개인식별정보를 직접 수집하지 않음으로써 국내 정보통신망법 및 COPPA 규정을 철저히 준수합니다.
   - **쿠키 정책 및 동의:** Google Analytics 등 마케팅 분석 툴 활용 시 쿠키 수집 사실을 명시하고 사용자 동의 배너를 제공합니다.
   - **데이터 파기:** 보관 기한이 경과한 회원 탈퇴 기록 및 무효화된 토큰, 로그는 즉시 파기합니다.
...
45. **GA 분석 이슈 접근 원칙 — 목적 먼저 파악 (재발 방지)**:
    - GA 커스텀 디멘션이나 이벤트 파라미터가 `(not set)`으로 잡히는 증상을 보더라도, **즉각 코드 수정으로 달려가기 전에 "사용자의 실제 분석 목적이 무엇인지"를 반드시 먼저 파악**해야 한다.
    - **검색어 분석 목적이라면 코드 수정 불필요**: GA4 `pageLocation` 차원에서 `?q=` URL 파라미터를 파싱하면 이미 모든 검색어가 수집된다. `customEvent:keyword`가 제대로 안 잡혀도 `pageLocation` 파싱으로 동일한 데이터를 얻을 수 있으므로 코드 변경이 필요 없다.
