...
27. **네이버 블로그 원고 자동 생성기 구현 및 스케줄링**:
    ...
    - 요일별 큐레이션 자동 매핑(KST 기준 월:1번째, 수:2번째, 금:3번째, 그 외:월요일 큐레이션 폴백)을 지원하며, 월/수/금 오전 9시 KST에 원고가 사전 생성되어 아티팩트32. **데이터베이스 쿼리 및 캐싱 성능 최적화 규격**:
...
34. **지능형 뒤로가기(Smart Back Route) 구현 규격**:
    ...
    - **헤더 좌측 단순화 동기화**: 상세 페이지(`BookDetailClient.tsx`)와 목록 페이지(`BooksPageClient.tsx`)의 `PageHeader` 좌측에는 주렁주렁 홈 버튼을 추가하는 대신, 스마트 백 버튼(`<BackButton href="/" />` 등)만 단독 배치하여 구조를 100% 동기화한다.ation_noise.py` 등을 실행해 정제하되, 정제 이후에도 각 테마의 도서 수량이 7권 이상으로 유지되는지 안전장치를 확인해야 한다.
