...
for i, book in enumerate(successful_books, 1):
    ...
    try:
        ...
        # 패턴: fnSearchResultDetail(1949734267,1949734269,'BO')
        onclick = link.get('onclick', '')
        match = re.search(r'fnSearchResultDetail\((\d+),(\d+),', onclick)
        ...
        if not match:
            ...
            fail_count += 1
            print(f"  ❌ recKey/bookKey 추출 실패: {onclick}")
            continue
        
        rec_key = match.group(1)
        book_key = match.group(2)
