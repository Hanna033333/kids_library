...
def run_this_week_analysis():
    client = BetaAnalyticsDataClient()
    property_id = "518474196"
    
    # "This week" starting from Monday, April 13, 2026
    start_date = "2026-04-13"
    end_date = "yesterday"
    
    report_lines = []
    report_lines.append(f"\n{'='*70}")
    report_lines.append(f"📊 이번주 GA4 상세 리포트 (4/13 ~ 어제)")
    ...
    for row in response.rows:
        ...
        total_users += users
        total_views += views
    report_lines.append(f"\n   합계: 이번주 총 사용자 {total_users} (중복 포함 합산) | 총 뷰 {total_views}\n")
    ...
    # Save to file
    report_text = "\n".join(report_lines)
    output_file = os.path.join(script_dir, "ga_this_week_report.txt")
