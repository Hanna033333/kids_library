...
2. 카드뉴스 도서 요약(card_descriptions) 작성 지침:
    - 각 도서별로 3줄을 거의 꽉 채울 수 있도록 반드시 공백 포함 75자에서 85자 사이의 완성도 있는 텍스트로 작성하세요. (글자 수가 너무 짧아지면 카드뉴스에서 2줄만 노출되어 균형이 깨지므로, 반드시 75자~85자 범위를 맞춰 3줄을 꽉 채울 것)
   ...
   - 각 줄 끝에는 자연스럽게 마침표가 들어가는 완성된 문장 형태로 종결하세요. (예: "~하는 이야기입니다.", "~하게 됩니다.") 문장이 끊기거나 말줄임표(...)로 끝나서는 안 됩니다.
