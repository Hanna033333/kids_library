...
1. 본문 캡션(caption) 작성 지침:
   ...
   - **반드시 다음 2단락의 정확한 레이아웃 순서를 엄격히 준수하여 빈 줄 개행까지 똑같이 생성하세요.** 다른 불필요한 서두 멘트, 이정표, 꼬리표([본문] 등)는 절대로 붙여선 안 됩니다.
     --- (레이아웃 순서 시작) ---
...
{{
  "caption": "스레드 본문에 노출할 2단락 구조의 캡션 텍스트",
  "card_descriptions": [
...
1. 본문 캡션(caption) 작성 지침:
   ...
   - **반드시 다음 2단락의 정확한 레이아웃 순서를 엄격히 준수하여 빈 줄 개행까지 똑같이 생성하세요.**
     --- (레이아웃 순서 시작) ---
