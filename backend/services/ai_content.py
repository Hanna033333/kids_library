...
def generate_fallback_content(
    ...
    caption = (
        ...
        f"다음번에 도서관 가실 때 바로 찾아보실 수 있게 3권 정리해 둡니다. 📌"
    )
...
[작성 지침 - 초간결 저마찰 솔루션 확정형]
1. 본문 캡션(caption) 작성 지침:
   - **글자 수 및 라인 수 엄격 제어**: 본문 캡션은 반드시 공백 포함 100자에서 120자 이내, 정확히 3줄(3문장)로 작성하세요. (모바일 화면에서 '더보기' 클릭 없이 한눈에 들어오는 초간결 분량)
   - **반드시 다음 3행 구조를 엄격히 준수하여 행간 개행(빈 줄)까지 맞춰 생성하세요:**
     ...
     (빈 줄)
     3행: [현실 육아 상황(시간/장소)에 100% 부합하는 솔루션 가치 확정 1줄 마무리 (질문 절대 금지, 이모지 1개 포함)]
          - 수면/잠자리 테마 ➔ "오늘 밤 아이와 잠자리에서 함께 읽어보시면 참 편안해지실 거예요. 🌙"
          - 도서관/대출/나들이 테마 ➔ "다음번에 도서관 가실 때 바로 찾아보실 수 있게 3권 정리해 둡니다. 📌" 또는 "주말에 아이와 도서관 나들이 가실 때 꺼내보시라고 3권 모아둡니다. 📌"
   ...
   - **기타 금지 사항**: '안녕하세요, 책자리 입니다' 인사말 금지, URL 링크 금지, #해시태그 금지, 책 제목/출판사명 직접 노출 금지.
...
[작성 지침 - 초간결 저마찰 솔루션 확정형]
1. 본문 캡션(caption) 작성 지침:
   - 사용자의 피드백을 반영하되, 반드시 공백 포함 100자에서 120자 이내 초간결 3줄 구조(1행 훅 - 2행 솔루션 소개 - 3행 솔루션 확정 1줄 마무리)를 유지하세요.
   - 3행 마무리는 질문형 문장을 절대 금지하며, 현실 시간/장소(잠자리 ➔ "오늘 밤 아이와 잠자리에서.. 🌙", 도서관 ➔ "다음번에 도서관 가실 때.. 📌")에 부합하는 확정형 문장으로 마쳐야 합니다.
   - static 인사말("안녕하세요"), '스하리', '댓글 달아주세요', URL, #해시태그, 책 제목 노출은 금지합니다.
