...
def generate_fallback_content(
    ...
    caption = (
        f"(저장하고 보세요📚) 자료나눔\n\n"
        f"우리 아이 도서 선택 때문에 고민 많으시죠?\n"
        f"저도 맨날 같은 고민..ㅎ\n\n"
        f"오늘의 추천 큐레이션은 <{curation_title}> 입니다 두둥;\n\n"
        f"보자마자 감탄한 좋은 책들 모아봤어요.\n"
        f"좋은 건 함께 공유할게요!!\n\n"
        f"여러분만의 책육아 꿀팁이나 노하우가 있다면 댓글로 함께 나눠요! 💬\n\n"
        f"👉 댓글로 '스하리' 남겨주시면 한번에 볼 수 있는 링크 보내드려요!"
    )
...
1. 본문 캡션(caption) 작성 지침:
   ...
   - **반드시 다음 4단계의 정확한 레이아웃 순서(2단락 구조)를 엄격히 준수하여 빈 줄 개행까지 똑같이 생성하세요.** 다른 불필요한 서두 멘트, 이정표, 꼬리표([본문] 등)는 절대로 붙여선 안 됩니다.
     ...
     (저장하고 보세요📚) 자료나눔
     (빈 줄)
     [단락 1: 양육자의 일상 고민이나 공감대를 자극하는 실제 육아 에피소드 인트로. 캐주얼하고 친근한 인터넷 구어체 사용. (예: "여름방학이 다가옵니다 두둥;", "저도 같은 고민..ㅎ") (3~4줄)]
     (빈 줄)
     [단락 2: 해당 고민을 함께 나누고 덜어줄 이번 주 큐레이션 테마의 기획 의도 및 도서들을 보며 감탄한 추천 의도 소개. (예: "보자마자 혼자 감탄했네요 ㅎ 좋은건 함께 공유할께요!!") (3~4줄)]
     (빈 줄)
     [단락 3 (소통 질문): 학부모들의 활발한 댓글 소통을 자극하는 테마 관련 질문 (1~2줄)]
     (빈 줄)
   ...
   - **말투 지침 (최중요)**: 격식 있는 사서체 대신, 육아 커뮤니티에서 흔히 쓰이는 **친근하고 다정한 존댓말 구어체**를 구사하세요. 문장 끝에 '..ㅎ', 'ㅎ', '두둥;', '공유할게요!!' 등의 자연스러운 표현을 적절히 섞어 친근한 3040 엄마/아빠 톤앤매너를 만드세요. 줄바꿈은 한두 문장 단위로 짧게 나누어 가독성을 확보해 주세요.
...
2. 카드뉴스 도서 요약(card_descriptions) 작성 지침 (비주얼 가이드):
    ...
    - **존댓말 종결 어미 필수 (단, "~이야기입니다" 기계적 반복 절대 금지)**: 모든 도서 설명이 획일적으로 "~이야기입니다."로 끝나지 않도록, 다채롭고 자연스러운 종결 어미(예: "~그려냅니다.", "~보여 줍니다.", "~담아냅니다.", "~전해 줍니다.", "~선사합니다.")를 다양하게 활용해 주세요. 반말(예: "~한다.", "~이야기.")로 끝나는 것은 여전히 엄격히 금지됩니다.
...
1. 본문 캡션(caption) 작성 지침:
   ...
   - **반드시 다음 4단계의 정확한 레이아웃 순서(2단락 구조)를 엄격히 준수하여 빈 줄 개행까지 똑같이 생성하세요.**
     ...
     (저장하고 보세요📚) 자료나눔
     (빈 줄)
     [단락 1: 피드백이 반영된 양육자의 일상 고민이나 공감대 에피소드 인트로 (친근한 인터넷 구어체 사용, 예: "저도 같은 고민..ㅎ")]
     (빈 줄)
     [단락 2: 피드백이 반영된 이번 주 큐레이션 테마 기획 의도 및 도서 안내 (예: "좋은건 함께 공유할께요!!")]
     (빈 줄)
     [단락 3 (소통 질문): 피드백을 반영하여 학부모들의 활발한 댓글 소통(핑퐁 답글)을 자극하는 해당 테마 관련 육아 질문 (1~2줄)]
     (빈 줄)
     👉 댓글로 '스하리' 남겨주시면 한번에 볼 수 있는 링크 보내드려요!
     --- (레이아웃 순서 끝) ---
