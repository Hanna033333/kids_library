...
def generate_fallback_content(
    ...
    caption = (
        ...
        f"여러분만의 추천 책육아 꿀팁이나 노하우가 있다면 댓글로 함께 나누어 주세요! 💬\n\n"
...
1. 본문 캡션(caption) 작성 지침:
   ...
   - **반드시 다음 4단계의 정확한 레이아웃 순서(2단락 구조)를 엄격히 준수하여 빈 줄 개행까지 똑같이 생성하세요.** 다른 불필요한 서두 멘트, 이정표, 꼬리표([본문] 등)는 절대로 붙여선 안 됩니다.
     ...
     (빈 줄)
     [단락 3 (소통 질문): 학부모들의 활발한 댓글 소통(핑퐁 답글)을 자극하는 해당 테마 관련 육아 질문 (예: "안 자려는 아이 재울 때 쓰는 나만의 치트키가 있나요?", "새학기 등원거부 극복 팁이 있다면 댓글로 알려주세요!") (1~2문장)]
     (빈 줄)
...
1. 본문 캡션(caption) 작성 지침:
   ...
   - **반드시 다음 4단계의 정확한 레이아웃 순서(2단락 구조)를 엄격히 준수하여 빈 줄 개행까지 똑같이 생성하세요.**
     ...
     (빈 줄)
     [단락 3 (소통 질문): 피드백을 반영하여 학부모들의 활발한 댓글 소통(핑퐁 답글)을 자극하는 해당 테마 관련 육아 질문 (1~2문장)]
     (빈 줄)
     👉 더 많은 독서 목록을 보시려면 '스하리' 해주세요!
     --- (레이아웃 순서 끝) ---
