...
def check_7_books_exist(tag: str) -> bool:
    ...
    if tag in SPECIAL_TAGS:
        query = query.ilike("curation_tag", f"%{tag}%")
    else:
        or_filter = f'curation_tag.eq."{tag}",curation_tag.like."{tag},%",curation_tag.eq."#{tag}",curation_tag.like."#{tag},%"'
        query = query.or_(or_filter)
    
    result = query.limit(7).execute()
    return len(result.data) >= 7 if result.data else False

def get_weekly_curations(now_date: datetime.date = None) -> list:
    """
    공유 JSON(weekly_schedule.json)을 로드하여 일치하는 일정이 있으면 리턴하고,
    일치하는 일정이 없으면 UTC 타임스탬프 일수 기반 LCG로 결정적 랜덤 큐레이션을 선출합니다.
    선출 시 7-Book Rule을 만족하는지 DB를 조회하여 검증합니다.
    """
    import os
    import json
    import time
