"""
460페이지부터 이어서 수집 (100권씩 배치, 각 배치마다 출력)
"""
...
def collect_in_batches(start_dt: str = "2010-01-01", end_dt: str = "2025-12-31", batch_size: int = 100, start_page: int = 460):
    """
    460페이지부터 이어서 수집 (100권씩 배치 처리)
    """
    print("=" * 60)
    print("판교 도서관 아동 도서 수집 (이어서 진행)")
    print(f"기간: {start_dt} ~ {end_dt}")
    print(f"시작 페이지: {start_page} (459페이지까지 완료)")
    print("=" * 60)
    print()
    sys.stdout.flush()
    ...
    print()
    print(f"⚠️  {start_page}페이지부터 {batch_size}권씩 수집하고 각 배치 완료 시 알림을 드립니다.")
    print()
    sys.stdout.flush()
...
if __name__ == "__main__":
    collect_in_batches(start_page=460)
